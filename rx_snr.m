...
for k=1:length(vtau)
    ...
    tau=-1-vtau(k);
    beta2=sinc(tau).*cos(pi*a*tau)./(1-4*a^2*tau^2);
    
    n=0:N-1;
    c=abs(alfa+beta*exp(-1i*2*pi*n/N)++beta2*exp(1i*2*pi*n/N)).^2;
    sig2(k,:)=N0*(1+AF*(g1+g2*c));
    gama(k,:)=AF*P0*(g1+g2*c)./(1+AF*(g1+g2*c));
...
legend('\tau=0','\tau=0.2','\tau=0.3','\tau=0.4','\tau=0.5')
